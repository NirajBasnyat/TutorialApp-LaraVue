<template>
    <div
        class="modal fade"
        id="login-modal"
        tabindex="-1"
        role="dialog"
        aria-labelledby="exampleModalLabel"
        aria-hidden="true"
    >
        <div class="modal-dialog" role="document">
            <div
                class="card card-shadowed p-50 w-400 mb-0"
                style="max-width: 100%"
            >
                <h5 class="text-uppercase text-center">Login</h5>
                <br /><br />
                <form>
                    <ul
                        class="list-group alert alert-danger"
                        v-if="errors.length > 0"
                    >
                        <p
                            class="text-center"
                            v-for="(error, i) in errors"
                            :key="i"
                        >
                            {{ error }}
                        </p>
                    </ul>
                    <div class="form-group">
                        <input
                            type="text"
                            class="form-control"
                            placeholder="Email"
                            v-model="email"
                        />
                    </div>

                    <div class="form-group">
                        <input
                            type="password"
                            class="form-control"
                            placeholder="Password"
                            v-model="password"
                        />
                    </div>

                    <div class="form-group flexbox py-10">
                        <label class="custom-control custom-checkbox">
                            <input
                                type="checkbox"
                                class="custom-control-input"
                                v-model="remember"
                            />
                            <span class="custom-control-indicator"></span>
                            <span class="custom-control-description"
                                >Remember me</span
                            >
                        </label>

                        <a class="text-muted hover-primary fs-13" href="#"
                            >Forgot password?</a
                        >
                    </div>

                    <div class="form-group">
                        <button
                            class="btn btn-bold btn-block btn-primary"
                            type="button"
                            :disabled="!isValidCredentials"
                            @click="attemptLogin"
                        >
                            Login
                        </button>
                    </div>
                </form>
                <p class="text-center text-muted fs-13 mt-20">
                    Don't have an account?
                    <a href="page-register.html">Sign up</a>
                </p>
            </div>
        </div>
    </div>
</template>

<script>
import axios from "axios";

export default {
    data() {
        return {
            email: "",
            password: "",
            remember: false,
            loading: false,
            errors: []
        };
    },

    methods: {
        attemptLogin() {
            this.loading = true;
            axios
                .post("/login", {
                    email: this.email,
                    password: this.password,
                    remember: this.remember
                })
                .then(resp => {
                    console.log(resp);
                })
                .catch(err => {
                    this.loading = false;
                    if (err.response.status == 422) {
                        this.errors.push(
                            "Your credentials do not match our records"
                        );
                    } else {
                        this.errors.push("Oops something went wrong !");
                    }
                    console.log(err);
                });
        },

        isEmailValid() {
            if (
                /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/.test(
                    this.email
                )
            ) {
                return true;
            }
            return false;
        }
    },

    computed: {
        isValidCredentials() {
            //if both email and password are valid send true
            return this.isEmailValid() && this.password && !this.loading; //should not load to be valid
        }
    }
};
</script>
